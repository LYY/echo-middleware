...
func typicalGetSet(t *testing.T, newCache cacheFactory) {
	...
	value := "foo"
	if err = cache.Set("value", value, DefaultExpiration); err != nil {
...
func incrDecr(t *testing.T, newCache cacheFactory) {
	...
	if err = cache.Set("int", int64(10), DefaultExpiration); err != nil {
...
func expiration(t *testing.T, newCache cacheFactory) {
	...
	// Test Set w/ DEFAULT
	value := 10
	cache.Set("int", value, DefaultExpiration)
	time.Sleep(2 * DefaultExpiration)
	...
	// Test Set w/ forever.
	cache.Set("int", value, StayForever)
	time.Sleep(2 * time.Second)
	err = cache.Get("int", &value)
	if err != nil {
		t.Errorf("Expected to get the value, but got: %s", err)
...
func testReplace(t *testing.T, newCache cacheFactory) {
	...
	if err = cache.Replace("notexist", 1, StayForever); err != ErrNotStored {
